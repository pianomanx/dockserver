...
LXC() {
  ...
  if [[ ! -f "/home/.lxcstart.sh" ]];then $(command -v rsync) /opt/dockserver/traefik/installer/subinstall/lxcstart.sh /home/.lxcstart.sh -aq --info=progress2 -hv;fi
  if [[ -f "/home/.lxcstart.sh" ]];then 
     $(command -v chmod) a=rx,u+w /home/.lxcstart.sh
     $(command -v bash) /home/.lxcstart.sh
     $(command -v ansible-playbook) /opt/dockserver/traefik/installer/subinstall/lxc.yml 1>/dev/null 2>&1
  fi
## set cron.d
  if [[ -f "/home/.lxcstart.sh" ]];then $(command -v ansible-playbook) /opt/dockserver/traefik/installer/subinstall/lxc.yml 1>/dev/null 2>&1;fi
