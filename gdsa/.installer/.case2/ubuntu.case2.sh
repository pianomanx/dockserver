...
SALT=${SALT:-NOT-SET}" >$basefolder/GDSA/.env
fi
}
projectname() {
basefolder="/opt/appdata"
...
if [[ $SABASENAME != "" ]];then
   if [[ $(uname) == "Darwin" ]];then
      sed -i '' "s/PROJECTNAME=NOT-SE/PROJECTNAME=$SABASENAME/g" $basefolder/GDSA/.env
   else
...
if [[ $PASSWORD != "" ]];then
   ...
   $(command -v docker) pull rclone/rclone -q > /dev/null
      sed -i '' "s/PASSWORD=NOT-SET/PASSWORD=$($(command -v docker) run rclone/rclone obscure ${PASSWORD} | tail -n1)/g" $basefolder/GDSA/.env
   else
      sed -i "s/PASSWORD=NOT-SET/PASSWORD=$($(command -v docker) run rclone/rclone obscure ${PASSWORD} | tail -n1)/g" $basefolder/GDSA/.env
   fi
else
...
if [[ $SALT != "" ]];then
   if [[ $(uname) == "Darwin" ]];then
      sed -i '' "s/SALT=NOT-SET/SALT=$($(command -v docker) run rclone/rclone obscure ${SALT} | tail -n1)/g" $basefolder/GDSA/.env
   else
      sed -i "s/SALT=NOT-SET/SALT=$($(command -v docker) run rclone/rclone obscure ${SALT} | tail -n1)/g" $basefolder/GDSA/.env
   fi
else
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
EOF
$(command -v docker) run --rm --volumes-from gcloud-config -v /opt/appdata/GDSA:/GDSA:rw ghcr.io/dockserver/docker-gdsa:latest
sleep 5 && clear 
