...
EOF
   ...
   for fc in ${forcepush};do
       if [[ ! -x $(command -v $fc) ]];then $(command -v apt) install $fc --reinstall -yqq 1>/dev/null 2>&1;fi
   done
...
for i in ${apps};do
   ...
   if [[ -d $basefolder/$i ]];then $(command -v rm) -rf $basefolder/$i;fi
   run=$($(command -v docker) ps -aq --format '{{.Names}}' | grep -x $i)
   if [[ $run == $i ]];then $(command -v docker) stop $i 1>/dev/null 2>&1;fi
...
if [[ -d $basefolder/${typed} ]];then $(command -v rm) -rf $basefolder/${typed};fi
run=$($(command -v docker) ps -aq --format '{{.Names}}' | grep -x ${typed})
if [[ $run == ${typed} ]];then $(command -v docker) stop ${typed} 1>/dev/null 2>&1;fi
